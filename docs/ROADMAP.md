...
## M8 · 阅读体验（对标 Kindle，未排期）

依据 [research/kindle.md](research/kindle.md)：把"版式"从主题里拆出来（字体 / 字号 / 粗细 / 行距 / 段距 / 行宽 / 对齐，每项可跟随主题），工具栏「Aa」面板就地实时预览，版式预设（紧凑 / 标准 / 舒适 / 大字 + 自定义），两端对齐 + 断词，字数胶囊「本章剩余」并按读者实际速度校准。不做：亮度、词距字距、Word Wise / X-Ray。翻页模式进长期想法。
