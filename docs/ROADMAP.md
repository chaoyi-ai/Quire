...
- [x] 输入辅助：列表续行、围栏闭合、Tab/Shift-Tab 缩进、⌘B/⌘I/⌘K/⌘E（⌘E 后改为 ⌃⇧`，与系统"使用所选内容查找"冲突）（表格 `|` 对齐格式化 → M4 备选）
...
- ➖ Quick Look 预览扩展（#55）**不做**（2026-09-19 决定，issue 已关）：2026-08-23 的 spike 证明纯 SwiftPM 手工组装的 .appex 系统不登记（`pluginkit` 不认、`qlmanage` 在 XPC 建连处失败），要做只能加一个 Xcode 工程；项目坚持纯 SwiftPM、不建 Xcode 工程（ADR-7），不为一个预览扩展破例。
- [ ] 发布 1.0
...
- [x] 文风检查（#75，0.5.1）：显示 → 文风检查（⌥⇧⌘D）。`StyleChecker` 内置英文（填充 / 冗余 / 陈词滥调）与自建中文词表（填充、冗余如"免费赠送"、套话如"众所周知 / 赋能 / 抓手"）；内置词表按段落语言分开查（中文段不套英文整词规则，反之亦然），用户规则不分语言；`~/Library/Application Support/Quire/style-rules.txt`（短语 / `-例外` / `/正则/` / `#`），显示 → 编辑文风规则… 生成模板。只画橙色虚线删除线 + 悬停提示类别，只处理可见段落 ± 一屏，不改文本

### 7.2 著作归属（Authorship）
- [x] 著作归属（#77，0.5.7）：编辑 → 著作归属（⇧⌘A）。`Authorship`（QuireCore）：作者表（我 / 粘贴 / AI / 引用 + 自定义，自动配色）+ UTF-16 区间，`apply(replacing:…)` 在 `NSTextStorage.willProcessEditing` 处增量维护（含撤销 / 重做）；键入归"当前作者"，粘贴归"粘贴"或"以作者粘贴"指定的作者；"标记选区为…"；存文件尾 `<!-- quire-authorship v1 hash=… {json} -->`，`MarkdownDocument.source` 永远是去掉注释块的正文（导出 / 复制 / 渲染天然剥离）；读入时哈希（FNV-1a）对不上就丢区间、留作者表并弹一次提示；编辑器按作者色 22% 底色只铺可见带；归属统计…；混合模式 / pandoc / URL scheme 这类整体替换路径按公共前后缀差异 `realign`（0.5.8）

### 7.3 链接与组织
- [x] `[[wikilink]]`（#78，0.5.2）：解析成 `quire-wiki:` 链接；点击在侧栏根索引里就近解析（同目录 > 子目录 > 父目录，不分大小写，可带路径，只在根内）；编辑器输入 `[[` 弹原生补全（模糊匹配文件名，选定补 `]]`）；显示 → 后退 / 前进（⌃⌘← / →）记录 wikilink、相对链接与侧栏打开的跳转；HTML 导出为纯文本
...
依据 [research/kindle.md](research/kindle.md)。核心：**版式与配色分层**（ADR-15）——主题管颜色与默认版式，用户版式覆盖在上面，两者正交。不做：亮度、词距字距、Word Wise / X-Ray；翻页模式进长期想法。

- [x] 阅读版式拆分（#86，0.8.0）：`ReadingLayout`（正文 / 代码字体、字号、粗细、行距、段距、行宽、对齐，每项零值 = 跟随主题）存 `reader.layout`（旧的三个 reader.* 键首次启动迁移）；叠在 `RenderOptions` 上，`RenderStyle` 合成时显式值 > 主题；两端对齐只给正文段落与引用并配断词（`hyphenationFactor` 0.9），列表 / 代码 / 标题不动；粗细只对有对应字重的字体生效，不做合成描边。工具栏「Aa」（显示 → 阅读版式… ⌥⌘A）弹出面板就地实时预览，滑杆 70 ms 去抖，走 `ThemeManager.refresh()` 同一条重建路径；设置窗口的「阅读版式」段与面板共用同一份数据
- [x] 版式预设（#87，0.8.0）：内置 紧凑 / 标准 / 舒适 / 大字 + 「存为预设…」命名保存（`reader.layoutPresets`）、删除；预设只含版式不含配色；面板顶部显示当前匹配的预设，改动后显示「自定义」
- [x] 字数胶囊「本章剩余」+ 按读者实际阅读速度校准（#88，0.8.1）：`ReadingProgress`（QuireCore）——章节 = 最近的标题到下一个级别 ≤ 它的标题；按块的源码行切片统计（行起点表一次扫描，1 MB ≈ 1 ms）。`ReadingTracker`（App）在阅读视图顶部块每变一次采样"读过多少标称分钟 / 用了多久"，3–180 s 之间、0.25–4× 之内的样本才采纳，滑动平均得倍率存 `reader.speed`，≥ 3 个样本视为已校准；全文与本章时间都按倍率算。胶囊点击明细里选「全文 / 本章剩余」（`reader.progressMode`）并可重置校准

## ✅ M9 · 工作区窗口（0.10.0）

依据 [research/window-chrome.md](research/window-chrome.md)：一个窗口 = 一个工作区，文档是标签不是窗口；标签条只在正文列、侧栏只在左列、工具栏左段与侧栏同宽；铬两种颜色。侧栏单击 = 临时标签、双击 / 编辑固定；⌘W 关标签、⌘⇧W 关窗口；退出后按工作区恢复。

## 长期想法（未排期）

- 翻页模式（空格 / → 翻一屏，页边对齐到行；复用打印分页），见 research/kindle.md §3.7
