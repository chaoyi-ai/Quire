...
> 状态更新于 2026-09-19（M5–M9 已发布后，版本号见 ROADMAP / CHANGELOG）。

| 分类 | iA Writer | Quire 现状 | 归属 |
|---|---|---|---|
| 环境 | 无 chrome 沉浸写作（无标题栏 / 按钮） | ✅（⌘⇧D） | 0.3.2 ✅ |
| 环境 | 源码模式标记出挑（hanging `#` / `-` / `>`） | ✅ | 0.3.2 ✅ |
| 环境 | 自家字体 Mono / Duo / Quattro | ✅（设置 → 编辑「下载 iA Writer 字体…」，不随包） | 0.3.6 ✅ |
| 环境 | Focus：句子 / 段落 / 打字机 | ✅（⌘D 循环） | 0.3.2 ✅ |
| 智能 | 词性高亮（6 语） | ✅（8 语；中文 NLTagger 不给词性，不着色） | 0.5.0 ✅ |
| 智能 | 文风检查 + 自定义规则 | ✅（含自建中文词表） | 0.5.1 ✅ |
| 智能 | 著作归属（人 / AI / 引用） | ✅（文件尾注释块，导出剥离） | 0.5.7 ✅ |
| 智能 | 统计（字词句、阅读时间、选区、目标） | 🟡 字词 / 阅读时间 / 选区 / 本章剩余 ✅；句数、写作目标 ❌ | 0.3.1 ✅ 基础 · 0.8.1 ✅ 本章剩余 · 句数 / 目标未做 |
| 智能 | 拼写 / 语法（系统） | ✅ | — |
| 组织 | `[[wikilink]]` 补全 / 就近解析 / 历史 | ✅ | 0.5.2 ✅ |
| 组织 | `#hashtag` 索引 | ✅（侧栏标签分组，`#a/b` 嵌套） | 0.5.3 ✅ · 0.6.0 |
| 组织 | 收藏 / 最近 / Smart Folders | ✅ 收藏 + 最近；Smart Folders ➖ | 0.5.3 ✅ · 0.7.0 ⌘P 最近 |
| 组织 | 内容块（transclusion，CSV → 表格） | ✅（`![[file]]`：md / csv / 图片） | 0.5.4 ✅ |
| 组织 | 全局搜索 + 查询语法 | 🟡 全文搜索 + 正则 ✅；`name:` / NEAR / AND OR 查询语法 ❌ | 0.3.5 ✅ |
| 组织 | 云 Library（iCloud / Dropbox …） | ➖（目录即库） | — |
| 预览 | 模板（Sans / Serif / Manuscript / GitHub） | ✅（10 套 JSON 主题） | — |
| 预览 | 数学 | ✅（SwiftMath） | 0.4.0 ✅ |
| 导出 | PDF 排版参数（页边距 / 页眉页脚 / 编号） | ✅（纸张 / 边距 / 页眉页脚模板 / 标题编号 / 不孤行） | 0.5.5 ✅ |
| 导出 | docx 原生 | ✅（pandoc 可选，导入 / 导出） | 0.4.9 ✅ |
| 导出 | 博客发布 | ➖ | — |
| 集成 | URL 命令、Apple Shortcuts | ✅（`quire://` + 4 个 App Intents） | 0.5.6 ✅ |
| 自动化 | 智能引号 / 破折号 | ✅（设置 → 阅读，默认关） | 0.4.6 ✅ |
| 自动化 | 表格公式 / 元数据变量 | ➖ | — |
