...
- 新增：字数统计（#65）。阅读 / 编辑窗格右下角胶囊显示「N 字 · M 分钟」，有选区时显示选区字数，点击弹出明细（字词 / 其中中文字 / 字符 / 行 / 阅读时间）。CJK 每字计一、拉丁按词计；与解析同一趟后台计算，1 MB 约 1.5 ms，新增 `stats/large-1mb` 基准门禁（< 5 ms）。设置 → 阅读 可关闭。
